...
def parse_verified_queries_block(block: str) -> list:
    if not block:
        return []
    queries = []
    # GET_DDL returns double-quoted question names
    pattern = r'""([^""]+)""\s+AS\s*\('
...
def parse_ddl(ddl: str) -> ParsedView:
    ...
    # Top-level comment (appears as standalone line with comment=)
    ...
    if ext_match:
        # Extract CA='...' value from inside the extension block
        ca_match = re.search(r"CA='(.*)'", ddl[ext_match.start():], re.DOTALL)
        if ca_match:
            ca_raw = ca_match.group(1)
            # DDL uses doubled double-quotes ("") for literal double-quotes in the JSON
...
def build_extension_json(selections: dict, left_view, right_view) -> str:
    """Dynamically build the 'with extension (CA=...)' block based on selected objects."""

    # Build lookup: for each extension table name -> its full metadata dict, per source
    ...
    # Determine which tables are selected and from which source
    selected_table_short_names = {}  # short_name -> "left"/"right"
    for key, choice in selections.items():
        if choice == "skip":
            continue
        parts = key.split("::", 1)
        if parts[0] == "table":
            # Table name in DDL is DB.SCHEMA.TABLE — short name is the last part
            full_name = parts[1] if len(parts) > 1 else ""
            short_name = full_name.split(".")[-1].upper()
            selected_table_short_names[short_name] = choice

    # Collect selected dimension/fact/metric names grouped by table
    selected_dims_by_table = {}   # table_short -> set of dim names (upper)
    ...
    # Collect selected relationship names
    selected_rel_names = {}  # rel_name_upper -> "left"/"right"
    for key, choice in selections.items():
        if choice == "skip":
            continue
        parts = key.split("::", 1)
    ...
    for short_name, source in selected_table_short_names.items():
        ...
        # Dimensions: filter to only selected ones, preserving time_dimensions split
        ...
        for dname in sel_dim_names:
            if dname in src_time_dims:
                ...
                # Dimension not found in source extension — add as regular dimension
        ...
        # Foreign keys: keep only those whose pkey_table is also selected
        ...
        # Filters: copy from source if present
    ...
    # Serialize: compact JSON, then escape " as "" for DDL single-quoted string
...
st.markdown("Select two semantic views to compare, review differences and generate SQL.")
...
if st.session_state.get("compared"):
    ...
    with hcol1:
        st.markdown(f"**LEFT: {left_view.view_name}**")
    with hcol2:
        st.markdown(f"**RIGHT: {right_view.view_name}**")
    with hcol3:
        st.markdown("**Choice**")
    ...
    if left_view.tables or right_view.tables:
        st.markdown("### Tables")
        display_comparison_section(compare_objects(left_view.tables, right_view.tables), "table", selections)

    # Relationships
    if left_view.relationships or right_view.relationships:
        st.markdown("### Relationships")
        display_comparison_section(compare_objects(left_view.relationships, right_view.relationships), "relationship", selections)

    # Facts
    if left_view.facts or right_view.facts:
        st.markdown("### Facts")
        display_comparison_section(compare_objects(left_view.facts, right_view.facts), "fact", selections)

    # Dimensions
    if left_view.dimensions or right_view.dimensions:
        st.markdown("### Dimensions")
        display_comparison_section(compare_objects(left_view.dimensions, right_view.dimensions), "dimension", selections)

    # Metrics
    if left_view.metrics or right_view.metrics:
        st.markdown("### Metrics")
        display_comparison_section(compare_objects(left_view.metrics, right_view.metrics), "metric", selections)

    # Verified Queries
    if left_view.verified_queries or right_view.verified_queries:
        st.markdown("### Verified Queries")
        display_comparison_section(compare_objects(left_view.verified_queries, right_view.verified_queries), "verified_query", selections)

    # SQL Generation
    st.markdown("---")
    st.markdown("## Generate SQL")

    new_view_name = st.text_input("New Semantic View name (e.g. DB.SCHEMA.NAME)", value="MY_DB.MY_SCHEMA.MERGED_VIEW")

    col_gen, col_exec = st.columns(2)
    with col_gen:
        if st.button("Generate SQL", type="primary"):
            sql = generate_create_sql(new_view_name, selections, left_view, right_view)
    ...
    if st.session_state.get("generated_sql"):
        ...
        with col_exec:
            if st.button("Execute in Snowflake"):
                try:
